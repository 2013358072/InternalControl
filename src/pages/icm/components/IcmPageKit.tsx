...
.icm-grid{display:grid;gap:12px}.cols-2{grid-template-columns:repeat(2,minmax(0,1fr))}.cols-3{grid-template-columns:repeat(3,minmax(0,1fr))}.cols-4{grid-template-columns:repeat(4,minmax(0,1fr))}
...
.icm-link{border:0;background:transparent;color:#155bd4;font-weight:700;cursor:pointer;padding:0;font-size:12px}
...
export function Card({
  ...
  return (
    <section className="icm-card">
      ...
      <div className="icm-card-body">{children}</div>
    </section>
  )
}

export function Kpi({ value, label, note, tone = 'blue' }: { value: string; label: string; note: string; tone?: Tone }) {
  return (
    <div className={`icm-card icm-kpi tone-${tone}`}>
      <strong>{value}</strong>
      <span>{label}</span>
      <b>{note}</b>
    </div>
